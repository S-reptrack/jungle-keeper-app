...
const consistencyOptions = [
  { value: "normal", label: "Normale", color: "bg-green-500/20 text-green-700 dark:text-green-400 border-green-500/30" },
  { value: "liquid", label: "Liquide", color: "bg-yellow-500/20 text-yellow-700 dark:text-yellow-400 border-yellow-500/30" },
  { value: "hard", label: "Dure", color: "bg-orange-500/20 text-orange-700 dark:text-orange-400 border-orange-500/30" },
  { value: "abnormal", label: "Anormale", color: "bg-red-500/20 text-red-700 dark:text-red-400 border-red-500/30" },
];
...
const BowelSection = ({ reptileId, readOnly = false }: BowelSectionProps) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success("Selle enregistrée avec succès");
      setDialogOpen(false);
  ...
  const handleDelete = async (recordId: string) => {
    const confirmDelete = window.confirm("Supprimer cet enregistrement ?");
      ...
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
        ...
        {!readOnly && (
          <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Enregistrer une selle</DialogTitle>
              </DialogHeader>
              <div className="space-y-4 pt-2">
                <div>
                  <Label htmlFor="bowel-date">Date</Label>
                  <Input
                    id="bowel-date"
                    type="date"
                    value={bowelDate}
                ...
                <div>
                  <Label htmlFor="consistency">Consistance</Label>
                ...
                <div>
                  <Label htmlFor="bowel-notes">Notes (optionnel)</Label>
                  <Textarea
                    id="bowel-notes"
                    placeholder="Ex: Urates blanches, consistance normale..."
                    value={notes}
      ...
      <CardContent>
        {records.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-muted-foreground">Aucune selle enregistrée</p>
            {!readOnly && (
              <p className="text-sm text-muted-foreground mt-1">
                Cliquez sur "Ajouter" pour commencer le suivi
              </p>
            )}
          </div>
        ) : (
